...
public class ChuanYun_JieXie_YanCang_Activity extends BaseActivity implements
		OnClickListener {
	...
	// 标识是进行到了那个步骤
	private int step = 1;
	private String yc_type = "2";// 1是发运验舱，2是接卸验舱
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				if (msg.obj != null) {
					ToastUtil.ShowMessage(
							ChuanYun_JieXie_YanCang_Activity.this, "加载成功");

					chuanCangInfo_list = (List<ChuanCangInfo>) msg.obj;
				} else {
					ToastUtil.ShowMessage(
							ChuanYun_JieXie_YanCang_Activity.this, "加载失败");
				}
			...
			case 2:
				// 更改验舱步骤
				loadingDismiss();
				ToastUtil.ShowMessage(ChuanYun_JieXie_YanCang_Activity.this,
						"提交成功");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_chuanyun_jiexie_yancang);
		InitView();
		loading = new DialogLoading(ChuanYun_JieXie_YanCang_Activity.this);
			...
			public void run() {
				try {
					...
					Result result = ApiResult.getServerArrayResult(
							ChuanYun_JieXie_YanCang_Activity.this,
	...
	private void setView() {
		...
		if (step == 1) {
			...
			for (int i = 0; i < imgSites.size(); i++) {
				if ("331_1".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera_shuichidan, 1);
				} else if ("331_2".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera1, 1);

				} else if ("331_3".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera2, 1);

				} else if ("331_4".equals(imgSites.get(i).get("position"))) {
			...
			for (int i = 0; i < imgSites.size(); i++) {
				if ("332_2".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera1, 1);

				} else if ("332_3".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera2, 1);

				} else if ("332_4".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera3, 1);
				}
			}
		} else if (step == 3) {
			...
			for (int i = 0; i < imgSites.size(); i++) {
				if ("333_1".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera_shuichidan, 1);
				} else if ("333_2".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera1, 1);

				} else if ("333_3".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera2, 1);

				} else if ("333_4".equals(imgSites.get(i).get("position"))) {
					ChangeViewUtil.changeicon(iv_chuanyun_camera3, 1);
				}
			}
		}
	...
	public void onClick(View v) {
		...
		intent = new Intent(ChuanYun_JieXie_YanCang_Activity.this,
				TakePicAndUpload_Activity.class);
		switch (v.getId()) {
		case R.id.common_top_left:
			new TuiChuDialogUtil(ChuanYun_JieXie_YanCang_Activity.this)
					.showDialog();
			break;
		case R.id.iv_chuanyun_camera_shuichidan:
			if (step == 1) {
				camera_position = "331_1";
				business = "fengc_shuicdpz";
			} else if (step == 3) {
				business = "kongc_shuicdpz";
				camera_position = "333_1";
			}
					...
					.setPositiveButton("拍照",
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface arg0,
									...
									intent.putExtra("yw_type", "chuanyjx");
		...
		case R.id.iv_chuanyun_camera1:
			if (step == 1) {
				camera_position = "331_2";
				business = "fengc";
			} else if (step == 2) {
				camera_position = "332_2";
				business = "shic";
			} else if (step == 3) {
				camera_position = "333_2";
				business = "kongc";
			}
			dialog = new AlertDialog.Builder(
					ChuanYun_JieXie_YanCang_Activity.this)
		...
		case R.id.iv_chuanyun_camera2:
			if (step == 1) {
				camera_position = "331_3";
				business = "fengc";
			} else if (step == 2) {
				camera_position = "332_3";
				business = "shic";
			} else if (step == 3) {
				camera_position = "333_3";
				business = "kongc";
			}
			dialog = new AlertDialog.Builder(
					ChuanYun_JieXie_YanCang_Activity.this)
		...
		case R.id.iv_chuanyun_camera3:
			if (step == 1) {
				camera_position = "331_4";
				business = "fengc";
			} else if (step == 2) {
				camera_position = "332_4";
				business = "shic";
			} else if (step == 3) {
				camera_position = "333_4";
				business = "kongc";
			}
			dialog = new AlertDialog.Builder(
					ChuanYun_JieXie_YanCang_Activity.this)
